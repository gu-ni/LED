...
from TAAE.model_TAE import reparameterize
...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            cond = next(self.data)
            with th.no_grad():
                in_x = cond['input_id_x'].permute(1, 0)
                in_y = cond['input_id_y'].permute(1, 0)
                mu_x, logvar_x, _ = self.tae_model.encode(in_x)
                mu_y, logvar_y, _ = self.tae_model.encode(in_y)
                z_x = reparameterize(mu_x, logvar_x)
                z_y = reparameterize(mu_y, logvar_y)
                
                batch = th.cat([z_x, z_y], dim=0).permute(1, 0, 2).contiguous()
                del cond['input_id_x'], cond['input_id_y'], cond['input_ids']
            ...
            if self.eval_data is not None and self.step % self.eval_interval == 0:
                ...
                with th.no_grad():
                    in_x = cond_eval['input_id_x'].permute(1, 0)
                    in_y = cond_eval['input_id_y'].permute(1, 0)
                    mu_x, logvar_x, _ = self.tae_model.encode(in_x)
                    mu_y, logvar_y, _ = self.tae_model.encode(in_y)
                    z_x = reparameterize(mu_x, logvar_x)
                    z_y = reparameterize(mu_y, logvar_y)
                    
                    batch_eval = th.cat([z_x, z_y], dim=0).permute(1, 0, 2).contiguous()
                    del cond_eval['input_id_x'], cond_eval['input_id_y'], cond_eval['input_ids']
