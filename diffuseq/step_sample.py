...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        ...
        """
        ## self.weights(): diffusion step 하나 당 weight 하나.
        ## w: 1이 2000개 있는 벡터. 즉 모든  diffusion step에 대한 가중치가 동일함.
        w = self.weights()
        ## p: w를 가중치로 바꿈.
        p = w / np.sum(w)
        ## indices_np: 0~1999 사이의 정수를 19개 선택. 이때 각 2000개의 정수가 선택될 확률을 p로 정의함.
        indices_np = np.random.choice(len(p), size=(batch_size,), p=p)
        indices = th.from_numpy(indices_np).long().to(device)
        ## weights_np: 길이 19인 벡터. p[indices_np]가 클수록 weights_np는 작은 값을 가짐.
