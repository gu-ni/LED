...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            #두 문장을 더한 길이가 127(=128-1)을 넘을 때, 127보다 같거나 작아질 때까지 src 문장의 맨 뒤 단어를 하나씩 뺌. 어차피 데이터셋 중 가장 긴 문장의 길이가 60이라 상관 없음
            while len(src) + len(trg) > seq_len - 1:
                src.pop()
            
            lst_X.append(src + [vocab_dict.sep_token_id])
            lst_Y.append(trg)
            lst.append(src + [vocab_dict.sep_token_id] + trg)
            # mask는 source 문장에만 0 할당.
            mask.append([0]*(len(src)+1) + [1]*(seq_len - (len(src)+1)))
        
        group_lst['input_id_x'] = lst_X
        group_lst['input_id_y'] = lst_Y
    ...
    def pad_function(group_lst):
        # _collate_batch_helper: max length(=128)만큼 문장 길이를 늘림
        group_lst['input_ids'] = _collate_batch_helper(group_lst['input_ids'], vocab_dict.pad_token_id, seq_len)
        group_lst['input_mask'] = _collate_batch_helper(group_lst['input_mask'], 1, seq_len)
        return group_lst
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    # jsonl 파일의 실제 문장들을 'input_id_x', 'input_id_y', 'input_ids', 'input_mask'의 key를 갖는 딕셔너리에 
